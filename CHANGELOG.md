...
## [1.2.1] - 2026-09-01
### Fixed
- **Mobile Android Gradle Build**: Added `apps/mobile/android/gradle/wrapper/gradle-wrapper.properties` (Gradle 8.10.2), `gradle.properties` (AndroidX + JVM memory args), and upgraded Android Gradle Plugin to `8.5.2` and Kotlin to `1.9.24` in `settings.gradle`. Added `--android-skip-build-dependency-validation` flag to `flutter build apk` and `appbundle` in CI to ensure uninterrupted Gradle execution across Flutter toolchain revisions.
...
- **Playwright E2E CI WebServer & Assertions**: Configured `webServer` (`pnpm start` on port 3000) in `playwright.config.ts` for CI execution, aligned browser targets with installed Chromium dependencies, and synchronized test assertions in `auth.spec.ts`, `full-journey.spec.ts`, and `labs.spec.ts` with actual page headers and interactive DOM elements.
