...
Write-Host "   KUBELAB 154-LAB RUNTIME & DECLARATIVE CERTIFICATION HARNESS   " -ForegroundColor Cyan
Write-Host "=================================================================" -ForegroundColor Cyan

# 1. Run full schema validation on all 154 labs
Write-Host "`n[1/6] Executing schema validation for all 154 declarative YAML labs..." -ForegroundColor Yellow
$sw = [System.Diagnostics.Stopwatch]::StartNew()

$out = cargo run -p kubelab-validation-engine --bin validate_lab_schema -- --path "$PSScriptRoot/../labs" 2>&1
if ($LASTEXITCODE -eq 0) {
    Write-Host "      [PASS] 154/154 declarative YAML lab definitions valid and certified." -ForegroundColor Green
} else {
...
Write-Host "`n[2/6] Executing live evaluator assertions & negative condition test suite..." -ForegroundColor Yellow
...
Write-Host "  154 LAB RUNTIME CERTIFICATION COMPLETED ($($sw.ElapsedMilliseconds)ms) " -ForegroundColor Green
