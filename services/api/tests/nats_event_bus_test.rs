...
async fn test_nats_domain_events_pub_sub() {
    ...
    publisher.emit_lab_started(&event).await.expect("Emit lab started must succeed");
    ...
    let received: LabStartedEvent = serde_json::from_slice(&msg.payload).expect("Deserialize event payload");
